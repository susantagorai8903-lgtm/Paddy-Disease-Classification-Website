...
def main(max_images=None):
    try:
        ...
        # Safely print classification report. It's possible some classes
        # in `encoder.classes_` are not present in the test split (y_test),
        # which would cause classification_report to raise a ValueError.
        try:
            test_labels = np.unique(y_test)
            test_label_names = encoder.inverse_transform(test_labels)
            print("\nClassification Report:\n", classification_report(
                y_test, y_pred, labels=test_labels, target_names=test_label_names
            ))
        except Exception as cre:
            # Fallback: print a more tolerant report and warn the user.
            print("Warning: Failed to produce full classification report:", cre)
